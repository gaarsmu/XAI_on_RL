    ...
    def play_game(self, agent):
        ...
        while True:
            ...
            for board_s, probs_s in getBoardSims(env.getPBoard(), probs):
                train_samples.append([board_s, probs_s, env.fpt])
            
            action = np.random.choice(probs.shape[0], p=probs.reshape(-1,))
            s = env.getBoardHash()
            valid_actions = env.getValidMoves()
            _, reward, done, _ = env.step( (action//env.size, action % env.size) )
            if done:
                final_player = env.fpt
                return [ (x[0], x[1], reward if final_player==x[2] else -reward)  for x in train_samples]

    def execute_update(self, agent, mcts_args):
        train_examples = []
        for _ in tqdm(range(self.num_eps)):
            self.mcts = MCTS(agent, mcts_args)
            train_examples += self.play_game(agent)

        agent.updateNet(np.array(train_examples))
